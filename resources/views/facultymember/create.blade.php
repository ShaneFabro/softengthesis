...
@section('content')
    ...
    <form action="{{ isset($particular) ? route('member.update', $particular->user_id) : route('member.store') }}" method="POST" enctype="multipart/form-data">
        @csrf
        @if(isset($particular))
            @method('PUT')
        @endif
        ...
        <div class="form-group">
            <label for="mobilephone">Mobilephone Number:</label>
